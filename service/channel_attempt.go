...
// ChannelRouteAttempt records one upstream try for admin-only log metadata.
// It never carries request bodies, keys, or prompts.
...
// BeginChannelRouteAttempt starts tracking the attempt now being dispatched.
...
// FinishChannelRouteAttempt closes the in-flight attempt with its outcome and
// the retry decision that was made for it.
...
// FinishSuccessfulChannelRouteAttempt closes the current attempt as a success.
...
// GetChannelRouteAttempts returns a copy for admin-only log metadata.
// includeCurrent also snapshots an in-flight attempt as successful, because
// consume logs can be written before the controller regains control.
...
// AppendChannelRouteAttemptsAdminInfo exposes the recorded attempts in
// admin-only log metadata.
