...
func TestDecideChannelFailure(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:             "pooled account failure on image retries before output",
		...
		{
			name:             "image group capability failure retries and counts circuit",
			path:             "/v1/images/edits",
			apiErr:           types.WithOpenAIError(types.OpenAIError{Type: "upstream_error", Code: "account_error", Message: "Image generation is not enabled for this group"}, http.StatusForbidden),
			retriesRemaining: 2,
			wantClass:        ChannelFailurePoolAccount,
			wantRetry:        true,
			wantEvict:        true,
		...
		{
			name:             "image pool exhaustion retries and counts circuit",
			path:             "/v1/images/edits",
			apiErr:           types.WithOpenAIError(types.OpenAIError{Type: "upstream_error", Code: "account_error", Message: "No available compatible accounts"}, http.StatusServiceUnavailable),
			retriesRemaining: 2,
			wantClass:        ChannelFailurePoolAccount,
			wantRetry:        true,
			wantEvict:        true,
		...
		{
			name:             "image 503 safely retries before output",
			path:             "/v1/images/generations",
			apiErr:           types.NewError(errors.New("unavailable"), types.ErrorCodeBadResponse, types.ErrOptionWithStatusCode(http.StatusServiceUnavailable)),
			retriesRemaining: 2,
			wantClass:        ChannelFailureTransient,
		...
		{
			name:             "image 500 safely retries on another channel",
			path:             "/v1/images/edits",
			apiErr:           types.NewError(errors.New("upstream did not accept the request"), types.ErrorCodeBadResponse, types.ErrOptionWithStatusCode(http.StatusInternalServerError)),
			retriesRemaining: 2,
			wantClass:        ChannelFailureTransient,
			wantRetry:        true,
			wantEvict:        true,
		...
		{
			name:             "image 524 safely retries on another channel",
			path:             "/v1/images/edits",
			apiErr:           types.NewError(errors.New("origin timeout before acceptance"), types.ErrorCodeBadResponse, types.ErrOptionWithStatusCode(524)),
			retriesRemaining: 2,
			wantClass:        ChannelFailureUncertain,
			wantRetry:        true,
			wantEvict:        true,
		...
		{
			name:             "image 524 does not retry after output starts",
			path:             "/v1/images/edits",
			apiErr:           types.NewError(errors.New("origin timeout before acceptance"), types.ErrorCodeBadResponse, types.ErrOptionWithStatusCode(524)),
			retriesRemaining: 2,
			responseStarted:  true,
		...
		{
			name:             "image 524 does not retry when fallback budget is exhausted",
			path:             "/v1/images/edits",
			apiErr:           types.NewError(errors.New("origin timeout before acceptance"), types.ErrorCodeBadResponse, types.ErrOptionWithStatusCode(524)),
			retriesRemaining: 0,
			wantClass:        ChannelFailureUncertain,
		...
		{
			name:             "image edits 502 safely retries before output",
			path:             "/v1/images/edits",
			apiErr:           types.NewError(errors.New("bad response status code 502"), types.ErrorCodeBadResponseStatusCode, types.ErrOptionWithStatusCode(http.StatusBadGateway)),
			retriesRemaining: 2,
			wantClass:        ChannelFailureTransient,
			wantRetry:        true,
			wantEvict:        true,
		...
		{
			name:             "image 429 safely retries before output",
			path:             "/v1/images/generations",
			apiErr:           types.NewError(errors.New("rate limited"), types.ErrorCodeBadResponse, types.ErrOptionWithStatusCode(http.StatusTooManyRequests)),
			retriesRemaining: 2,
			wantClass:        ChannelFailureRateLimited,
		...
		{
			name:             "image dial failure safely retries before request",
			path:             "/v1/images/edits",
			apiErr:           types.NewError(&net.OpError{Op: "dial", Net: "tcp", Err: errors.New("connection refused")}, types.ErrorCodeDoRequestFailed),
			retriesRemaining: 2,
			wantClass:        ChannelFailureTransient,
			wantRetry:        true,
			wantEvict:        true,
		...
		{
			name:             "image read failure remains protected",
			path:             "/v1/images/edits",
			apiErr:           types.NewError(&net.OpError{Op: "read", Net: "tcp", Err: errors.New("connection reset")}, types.ErrorCodeReadResponseBodyFailed, types.ErrOptionWithStatusCode(http.StatusBadGateway)),
			retriesRemaining: 2,
			wantClass:        ChannelFailureTransient,
			wantEvict:        true,
			wantCountCircuit: true,
		...
		{
			name:             "image 504 remains protected even when uncertain retry is allowed",
			path:             "/v1/images/edits",
			apiErr:           types.NewError(errors.New("upstream timeout"), types.ErrorCodeBadResponse, types.ErrOptionWithStatusCode(http.StatusGatewayTimeout)),
			retriesRemaining: 2,
			allowUncertain:   true,
			wantClass:        ChannelFailureUncertain,
			wantEvict:        true,
		...
		{
			name:             "image 503 does not retry after output starts",
			path:             "/v1/images/edits",
			apiErr:           types.NewError(errors.New("unavailable"), types.ErrorCodeBadResponse, types.ErrOptionWithStatusCode(http.StatusServiceUnavailable)),
			retriesRemaining: 2,
			responseStarted:  true,
