...
// StoreTemporaryReferenceMedia publishes validated media behind a short-lived,
// signed URL for upstream APIs that cannot consume multipart or data URLs.
func StoreTemporaryReferenceMedia(raw []byte, kind string) (string, error) {
	maxBytes := referenceMediaMaxBytes(kind)
	if maxBytes == 0 {
		return "", fmt.Errorf("unsupported reference media kind: %s", kind)
	}
...
func StoreTemporaryReferenceDataURL(dataURL, kind string) (string, error) {
	...
	return StoreTemporaryReferenceMedia(raw, kind)
}

func referenceMediaMaxBytes(kind string) int {
	switch kind {
	case "image":
...
func isSupportedReferenceMediaContentType(kind, mimeType string) bool {
	switch kind {
	case "image":
...
func ValidateTemporaryReferenceMediaSignature(id string, expires int64, signature string) bool {
	...
	return hmac.Equal([]byte(signature), []byte(generateReferenceMediaSignature(id, expires)))
}
