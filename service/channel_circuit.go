...
const (
	...
	channelHealthRecoveryStageDuration   = 10 * time.Second
...
func allowChannelHealthAttempt(c *gin.Context, channel *model.Channel, channelID int, modelName string, requestPath string) bool {
	...
	shard.Unlock()
	if c != nil {
		c.Set(ginKeyChannelHealthReservation, channelHealthReservation{Identity: identity, RecoveryStage: stage})
	}
	return true
}
...
func recordChannelCircuitFailure(c *gin.Context, channelID int, modelName string, requestPath string, class ChannelFailureClass, reason string, statusCode int) {
	...
	shard.Lock()
	state := getRouteHealthStateLocked(shard, identity, now)
	if state.RecoveryTargetCapacity > 0 && !identity.ImageGroup && !isImageGenerationPath(requestPath) && !reservation.RecoveryStage.Equal(state.LastRecoveryAt) {
		if state.InFlight > 0 {
			state.InFlight--
		}
		shard.Unlock()
...
func RecordChannelCircuitSuccess(c *gin.Context, channelID int, modelName string, requestPath string) {
	...
	if !imageRoute && state.RecoveryTargetCapacity > 0 && !reservation.RecoveryStage.Equal(state.LastRecoveryAt) {
		if state.InFlight > 0 {
			state.InFlight--
		}
		shard.Unlock()
	...
	if state.RecoveryTargetCapacity > 0 && !recovered {
		// Only requests admitted during this stage can advance recovery.
		if reservation.RecoveryStage.Equal(state.LastRecoveryAt) {
			state.RecoverySuccesses++
		}
		state.RecoveryFailures = 0
		if state.RecoverySuccesses >= channelHealthRecoverySuccessTarget &&
			now.Sub(state.LastRecoveryAt) >= channelHealthRecoveryStageDuration && state.Capacity < state.RecoveryTargetCapacity {
...
func HasDueChannelHealthProbe() bool {
	return hasDueChannelHealthProbe(nil)
}

func HasDueChannelHealthProbeForImages(images bool) bool {
	return hasDueChannelHealthProbe(func(imageRoute bool) bool { return imageRoute == images })
}

func hasDueChannelHealthProbe(includePath channelHealthProbePathFilter) bool {
	now := channelCircuitNow()
	...
	for index := range memoryChannelHealth.ChannelShards {
		...
		for _, state := range shard.States {
			...
			included := includePath == nil || includePath(isImageHealthRoute(state.ChannelID, state.Fingerprint, requestPath))
			if included && probeAvailable && !dueAt.IsZero() && !dueAt.After(now) && (state.Suspect || !state.OpenUntil.IsZero()) {
				shard.Unlock()
				return true
			}
	...
	for index := range memoryChannelHealth.RouteShards {
		...
		for _, state := range shard.Routes {
			...
			probeAvailable := !state.ProbeInFlight || !state.ProbeLeaseUntil.After(now)
			included := includePath == nil || includePath(isImageHealthRoute(state.ChannelID, state.Fingerprint, requestPath))
			if included && due && probeAvailable {
				shard.Unlock()
				return true
			}
		}
