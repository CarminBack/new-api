...
func GetAllDrawingLogs(startIdx int, num int, queryParams TaskQueryParams) []*Midjourney {
	limit := startIdx + num
	items := append(GetAllTasks(0, limit, queryParams), GetAllImageGenerationTasks(0, limit, queryParams, nil)...)
...
func GetAllUserDrawingLogs(userId int, startIdx int, num int, queryParams TaskQueryParams) []*Midjourney {
	limit := startIdx + num
	items := append(GetAllUserTask(userId, 0, limit, queryParams), GetAllImageGenerationTasks(0, limit, queryParams, &userId)...)
	return paginateDrawingLogs(items, startIdx, num)
}
...
func paginateDrawingLogs(items []*Midjourney, startIdx int, num int) []*Midjourney {
	slices.SortStableFunc(items, func(a, b *Midjourney) int {
	...
	return items[startIdx:min(startIdx+num, len(items))]
}
...
func GetAllImageGenerationTasks(startIdx int, num int, queryParams TaskQueryParams, userId *int) []*Midjourney {
	...
	for _, record := range records {
		item := imageGenerationToMidjourney(record)
		if userId != nil {
			item.ChannelId = 0
		}
