...
func TestConsumeCanvasOAuthCodeRequiresMatchingPKCEAndIsSingleUse(t *testing.T) {
	truncateTables(t)
	now := time.Now().Unix()
	code := &CanvasOAuthCode{CodeHash: "hash-single-use", UserId: 42, ClientId: "canvas", RedirectUri: "https://canvas.example/auth/callback", CodeChallenge: "challenge-correct", CreatedAt: now, ExpiresAt: now + 60}
...
func TestConsumeCanvasOAuthCodeConcurrentUseHasOneWinner(t *testing.T) {
	truncateTables(t)
	now := time.Now().Unix()
	code := &CanvasOAuthCode{CodeHash: "hash-concurrent", UserId: 42, ClientId: "canvas", RedirectUri: "https://canvas.example/auth/callback", CodeChallenge: "challenge", CreatedAt: now, ExpiresAt: now + 60}
	require.NoError(t, CreateCanvasOAuthCode(code))
