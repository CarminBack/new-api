...
func selectAbilitiesByRetry(abilities []Ability, retry int) []Ability {
	if len(abilities) == 0 {
		return abilities
	}
...
func GetChannelWithExclusions(group string, model string, retry int, requestPath string, imageResolutionTier string, exclusions ChannelKeyExclusions) (*Channel, error) {
	...
	abilities = selectAbilitiesByRetry(abilities, retry)
	if len(abilities) == 0 {
		return nil, nil
	}
...
func filterAbilitiesByImageResolution(abilities []Ability, model string, tier string) ([]Ability, error) {
	...
	for _, channel := range channels {
		supported, hasDeclaration := channel.GetSetting().ImageResolutionTierSupport(model, tier)
		if !hasDeclaration {
			continue
		}
		declared = true
		supportedChannels[channel.Id] = supported
	}
	if !declared {
		return abilities, nil
	}
	...
	filtered := make([]Ability, 0, len(abilities))
	for _, ability := range abilities {
		if supportedChannels[ability.ChannelId] {
			filtered = append(filtered, ability)
		}
	}
	return filtered, nil
