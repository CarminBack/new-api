...
func ModelPriceHelper(c *gin.Context, info *relaycommon.RelayInfo, promptTokens int, meta *types.TokenCountMeta) (hosttypes.PriceData, error) {
	if ratio_setting.IsTaskPerSecondBilling(info.OriginModelName) {
		return hosttypes.PriceData{}, fmt.Errorf("model %s requires a video task endpoint", info.OriginModelName)
	}
	if meta == nil {
	...
	imageGroupUnitPrice := meta.ImageGroupUnitPrice
	if isImagePricingGroup(info.UsingGroup) && imageGroupUnitPrice <= 0 {
		resolutionTier := common.GetContextKeyString(c, constant.ContextKeyImageResolutionTier)
		imageGroupUnitPrice, _ = ratio_setting.GetImageGroupPrice(resolutionTier)
	}
	if isImagePricingGroup(info.UsingGroup) && imageGroupUnitPrice > 0 {
		modelPrice = imageGroupUnitPrice
		usePrice = true
	...
	// Fixed image resolution prices take precedence over tiered text billing.
	if !imageUnitPriceOverride && billing_setting.GetBillingMode(info.OriginModelName) == billing_setting.BillingModeTieredExpr {
