...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	adaptor := GetTaskAdaptorForChannel(platform, info.ChannelBaseUrl)
	if adaptor == nil {
	...
	// 6. 将 OtherRatios 应用到基础额度。按次模型仅记录倍率，不参与扣费。
	...
	if adjustedRatios := adaptor.AdjustBillingOnSubmit(info, taskData); len(adjustedRatios) > 0 {
		if adjustedQuota, ok := recalcQuotaFromRatios(info, adjustedRatios, ratio_setting.IsTaskPerItemBilling(modelName)); ok {
...
func recalcQuotaFromRatios(info *relaycommon.RelayInfo, ratios map[string]float64, perCall bool) (int, bool) {
	if perCall {
...
func videoFetchByIDRespBodyBuilder(c *gin.Context) (respBody []byte, taskResp *dto.TaskError) {
	...
	if isOpenAIVideoAPI {
		...
		if originChannel, channelErr := model.CacheGetChannel(originTask.ChannelId); channelErr == nil {
			adaptor = GetTaskAdaptorForChannel(originTask.Platform, originChannel.GetBaseURL())
		}
		if adaptor == nil {
