...
export function RedemptionsMutateDrawer({
  ...
  useEffect(() => {
    if (!open || codeType !== 'invitation') return

    let ignoreResult = false
    setIsLoadingGroups(true)
    void getGroups()
      .then((result) => {
        if (!ignoreResult) setGroupOptions(result.data || [])
      })
      .catch(() => {
        if (!ignoreResult) setGroupOptions([])
      })
      .finally(() => {
        if (!ignoreResult) setIsLoadingGroups(false)
      })
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <Select
                            ...
                            value={field.value || ''}
                            onValueChange={field.onChange}
                            disabled={isUpdate || isLoadingGroups}
                          >
                            <FormControl>
                              <SelectTrigger className='w-full'>
                                <SelectValue
                          ...
                          <FormControl>
                            <ToggleGroup
                              ...
                              onValueChange={(values) => {
                                const current = field.value ? 'multi' : 'single'
