      ...
      <div className='w-full max-w-full min-w-0 space-y-2.5 overflow-x-hidden py-1 sm:space-y-3'>
        ...
        {props.isAdmin && adminInfo?.request_diagnostic && (
          <DetailSection label={t('Request Diagnostic')}>
            <DetailRow
              label={t('Captured')}
              value={adminInfo.request_diagnostic.captured ? t('Yes') : t('No')}
            />
              ...
              <div className='relative min-w-0'>
                <Button
                  ...
                  onClick={() =>
                    copyToClipboard(
                      JSON.stringify(
                        adminInfo.request_diagnostic?.body,
                        null,
                        2
                      )
                    )
        ...
        {other?.stream_status &&
          (props.isAdmin || other.stream_status.status !== 'ok') && (
            <DetailSection label={t('Stream Status')}>
              ...
              {props.isAdmin && other.stream_status.terminal_event && (
                <DetailRow
                  label={t('Terminal Event')}
                  value={other.stream_status.terminal_event}
                  mono
                />
              )}
              {props.isAdmin && other.stream_status.usage_present != null && (
                <DetailRow
                  label={t('Usage Present')}
                  value={other.stream_status.usage_present ? t('Yes') : t('No')}
                />
              )}
              {props.isAdmin &&
              ...
              {props.isAdmin &&
                other.stream_status.sent_event_count != null && (
                  <DetailRow
                    label={t('Sent Events')}
                    value={String(other.stream_status.sent_event_count)}
                  />
                )}
              {props.isAdmin && other.stream_status.billing_status && (
                <DetailRow
                  label={t('Billing Status')}
                  value={other.stream_status.billing_status}
                  mono
                />
              )}
              {props.isAdmin &&
                other.stream_status.downstream_started != null && (
                  <DetailRow
                    label={t('Downstream Started')}
                    value={
                      other.stream_status.downstream_started
                        ? t('Yes')
                        : t('No')
                    }
                  />
        ...
        {props.isAdmin &&
          ...
          other.admin_info.route_attempts.length > 0 && (
