...
func processChannelErrorWithDecision(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError, decision service.ChannelFailureDecision, recordErrorLog ...bool) {
	...
	if constant.ErrorLogEnabled && types.IsRecordErrorLog(err) && (len(recordErrorLog) == 0 || recordErrorLog[0]) {
		// 保存错误日志到mysql中
		userId := c.GetInt("id")
...
func decideTaskChannelFailure(c *gin.Context, taskErr *taskdto.TaskError, retryTimes int) service.ChannelFailureDecision {
	if taskErr == nil {
